...
constexpr char kMiniSdpSsrcGroupRtx[] = "FID";
constexpr char kMiniSdpSsrcGroupFec[] = "FEC-FR";
constexpr uint16_t kSdpMediaPortDefault = 9;  // webrtc set it to 9
constexpr uint16_t kSdpSctpPortDefault = 5000;
constexpr uint32_t kSdpSctpMaxMessageSizeDefault = 262144;
constexpr char kSdpApt[] = "apt";
...
// Transport Protocol
enum class TransportLayerProtocolType {
  kOverUdp = 0,
  kOverTcp
};
...
struct CandidateInfo {
  std::string Ip;
  uint16_t    Port;
  TransportLayerProtocolType TransportType;
};
...
class CodecDescription {
 ...
 public:
  ...
  uint8_t       AssociatePayloadType = 0;  // <apt>
...
class TrackDescription {
 ...
 public:
  ...
  bool IsStrictEqaul(const TrackDescription& rhs) const {
    return IsSimpleEqual(rhs) && attributes_ == rhs.attributes_;
  }
...
class MediaDescription {
 ...
 public:
  ...
  SdpMediaType  MediaType = SdpMediaType::kAudio;  // <media>
  ...
  // track: <ssrc> <Track>
  std::vector<TrackDescriptionPtr> Tracks;
 ...
 public:
  ...
  void SetRtxInfo(const std::map<uint8_t, uint8_t>& rtx_payload_info);
...
/**
 * @brief Make SessionDescription
 * 
 * @return SessionDescriptionPtr 
 */
inline SessionDescriptionPtr MakeSessionDescription() {
