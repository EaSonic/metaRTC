...
int main(int argc,char* argv[]){
    ...
    YangContext context; memset(&context,0,sizeof(context));
    yang_init_avinfo(&context.avinfo);
    yang_create_rtcContext(&context.rtc);
    context.avinfo.sys.mediaServer=Yang_Server_Whip_Whep;
    pc.peer.peerInfo.rtc.rtcLocalPort=context.avinfo.rtc.rtcLocalPort++;
    pc.peer.peerInfo.direction=YangRecvonly; pc.peer.peerInfo.uid=1; pc.peer.peerInfo.familyType=Yang_IpFamilyType_IPV4;
    pc.peer.peerCallback.recvCallback.context=NULL; pc.peer.peerCallback.recvCallback.receiveAudio=receiveAudio; pc.peer.peerCallback.recvCallback.receiveVideo=receiveVideo; pc.peer.peerCallback.recvCallback.receiveMsg=receiveMsg; context.sendRtcMessage.context=NULL; context.sendRtcMessage.sendRtcMessage=sendRtcMessage;
    memcpy(&pc.peer.peerCallback.rtcCallback,&context.rtcCallback,sizeof(YangRtcCallback));
    yang_create_peerConnection(&pc);
    pc.addAudioTrack(&pc.peer,(YangAudioCodec)context.avinfo.audio.audioDecoderType);
    ...
    context.rtc.enableRtpDump=yangtrue;
