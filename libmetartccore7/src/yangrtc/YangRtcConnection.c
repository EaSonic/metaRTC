...
static void g_yang_doTask(int32_t taskId, void *user) {
	...
	if (session->startRecv&&session->play) {
		...
		if (taskId == 1) {
			...
			if (session->context.streamConfig->streamDirection==YangRecvonly && session->play->send_rtcp_xr_rrtr(&session->context,session->play->pullStream))
...
static void yang_rtcconn_startudp(YangRtcSession *session) {
	yang_rtcconn_init(session, session->context.peerInfo->direction);
...
static void yang_rtcconn_receive(YangRtcSession *session, char *data, int32_t size) {
	...
	if(session->context.state == Yang_Conn_State_Connecting) {
		...
		if (session->context.peerCallback&&session->context.peerCallback->rtcCallback.sendRequest)
...
static int32_t yang_rtcconn_turn_sendData(YangRtcSocketSession *psession, char *data, int32_t nb) {
	...
	conn=(YangRtcConnection*)psession->user;
...
static int32_t yang_rtcconn_createOffer(YangRtcSession *session, char **psdp){
	...
	role=session->context.peerInfo->direction;

	if((err=session->ice.initIce(&session->ice.session))!=Yang_Ok){
...
static int32_t yang_rtcconn_setRemoteDescription(YangRtcSession* session,char* sdpstr){
	...
	if(session->context.peerInfo->mediaServer==Yang_Server_P2p && session->ice.session.iceState==YangIceFail) {
	...
	if(session->context.peerInfo->mediaServer==Yang_Server_P2p){
		...
		if((err=session->ice.iceHandle(&session->ice.session,session,yang_rtcconn_turn_receive,
				session->context.peerInfo->remoteIp,session->context.peerInfo->remotePort))!=Yang_Ok){
			return yang_error_wrap(err,"setLocalDescription error!");
		}


		if(session->ice.session.candidateType==YangIceTurn&&session->ice.session.iceState==YangIceSuccess){
			err=yang_create_rtcsocket(session->context.sock,session->context.peerInfo->familyType,Yang_Socket_Protocol_Udp,session->context.peerInfo->rtc.rtcLocalPort);
	...
	if(!session->isControlled)
		session->context.sock->updateRemoteAddress(&session->context.sock->session,session->context.peerInfo->remoteIp,session->isControlled?0:session->context.peerInfo->remotePort);
...
int32_t yang_create_rtcConnection(YangRtcConnection* conn,YangPeerInfo* peerInfo,YangPeerCallback* peerCallback){
	...
	yang_create_ice(&session->ice,peerInfo,peerCallback);
	...
	if(peerInfo->remotePort==0)
		peerInfo->remotePort=8000;
