...
static void  yang_sdp_genLocalSdp_candidate(YangRtcSession *session,YangMediaDesc *media_desc,char* localIp,int32_t localport){
    int32_t index;

	//candidate
    yang_insert_YangCandidateVector(&media_desc->candidates, NULL);
    index=media_desc->candidates.vsize-1;
    yang_strcpy(media_desc->candidates.payload[index].ip,localIp);
    media_desc->candidates.payload[index].port = localport;

    media_desc->candidates.payload[index].socketProtocol=session->context.peerInfo->rtc.rtcSocketProtocol;
...
static void yang_sdp_init_direction(YangMediaDesc* media_desc,YangRtcDirection role){
...
int32_t yang_sdp_genLocalSdp2(YangRtcSession *session, int32_t localport,char *dst, YangRtcDirection role) {
	int32_t a;
	int32_t midNum=0;
	int32_t mediaServer=session->context.peerInfo->mediaServer;
	...
	YangMediaPayloadType *videotype=NULL;
...
/*	yang_create_YangExtmapVector(&audio_media_desc->extmaps);
	...
	audiotype->clock_rate = session->context.peerInfo->pushAudio.sample; //48000;
	yang_itoa2(session->context.peerInfo->pushAudio.channel,audiotype->encoding_param, 10);


	yang_strcpy(audiotype->format_specific_param,
						session->context.peerInfo->pushAudio.enableAudioFec?"minptime=10;useinbandfec=1":"minptime=10");	//yang_create_stringVector(&audiotype->rtcp_fb);
	...
	videotype =&video_media_desc->payload_types.payload[0];
   ...
   if(mediaServer==Yang_Server_P2p){
	    ...
	    yang_create_YangCandidateVector(&video_media_desc->candidates);
	   	...
	   	char* localip=session->context.localIp;
	   	int32_t hasIplist=0;

	   	if(session->ice.session.candidateType==YangIceStun&&session->ice.session.iceState==YangIceSuccess){
...
static YangRtcTrack* yang_sdp_find_track(YangRtcSession* session,uint32_t ssrc){
	int32_t i;
...
int32_t yang_sdp_parseRemoteSdp(YangRtcSession* session,YangSdp* sdp){
	...
	for(i=0;i<sdp->media_descs.vsize;i++){
		...
		if(yang_yang_strcmp(desc->type,"audio")==0){
			...
			if(session->context.peerInfo->mediaServer==Yang_Server_P2p){
				if(desc->sendonly){
					session->context.peerInfo->direction=YangRecvonly;
				}else if(desc->recvonly){
					session->context.peerInfo->direction=YangSendonly;
				}else if(desc->sendrecv){
					session->context.peerInfo->direction=YangSendrecv;
				}
			}
			...
			for(k=0;k<desc->payload_types.vsize;k++){
				...
				//for(j=0;j<payload->rtcp_fb.vsize;j++){
		...
		if(desc->candidates.vsize>0){
			...
			for (k = 0; k < desc->candidates.vsize; k++) {
				...
				if(yang_memcmp(session->context.peerInfo->remoteIp,candidate->ip,yang_strlen(candidate->ip))==0){
					replaceIp=yangfalse;
					break;
				}
			}
			for (k = 0; k < desc->candidates.vsize; k++) {
				...
				if(candidate){
					if(candidate->port>0) {
						session->context.peerInfo->remotePort=candidate->port;
						if(replaceIp){
							yang_memset(session->context.peerInfo->remoteIp,0,sizeof(session->context.peerInfo->remoteIp));
							yang_strcpy(session->context.peerInfo->remoteIp,candidate->ip);
							replaceIp=yangfalse;
						}
					}
	...
	if(session->context.peerCallback&&session->context.peerCallback->rtcCallback.setMediaConfig){
...
#endif
		session->context.peerCallback->rtcCallback.setMediaConfig(session->context.peerCallback->rtcCallback.context,session->context.peerInfo->uid,
				remoteAudio,
				remoteVideo);
	}
	return Yang_Ok;
